...
class ArrayLocations extends ArrayList<iocomms.subpos.SPSData>
{
    ArrayList<iocomms.subpos.SPSData> locations = new ArrayList<iocomms.subpos.SPSData>();
    ...
    @Override
    public iocomms.subpos.SPSData get(int index)
    {
        return locations.get(index);
    }

    @Override
    public boolean add(iocomms.subpos.SPSData x)
    {
        iocomms.subpos.SPSData temp;
...
public class NodeLocations {
    ...
    iocomms.subpos.Position currentNodePosition = null;
    iocomms.subpos.Position currentPosition = null;
    private boolean calculated = false;

    public ArrayList<iocomms.subpos.SPSData> getLocations()
    {
    ...
    {
        iocomms.subpos.SPSData location = new iocomms.subpos.SPSData(averages);
    ...
    public void calculatePosition() {
        if (locations.size() > 0) {
            if (locations.size() == 1)
            {
                currentPosition = new iocomms.subpos.Position(locations.get(0).lat, locations.get(0).lng,
    ...
    private iocomms.subpos.Position trilaterate(ArrayLocations locations) {
        ...
        iocomms.subpos.TrilaterationFunction trilaterationFunction = new iocomms.subpos.TrilaterationFunction(positions, distances);
        iocomms.subpos.NonLinearLeastSquaresSolver solver = new iocomms.subpos.NonLinearLeastSquaresSolver(trilaterationFunction, new LevenbergMarquardtOptimizer());
        ...
        try {
            ...
            trilaterationFunction = new iocomms.subpos.TrilaterationFunction(err_positions, distances);
            solver = new iocomms.subpos.NonLinearLeastSquaresSolver(trilaterationFunction, new LevenbergMarquardtOptimizer());
        ...
        return new iocomms.subpos.Position(WebMercator.yToLatitude(optimum.getPoint().toArray()[0]),
